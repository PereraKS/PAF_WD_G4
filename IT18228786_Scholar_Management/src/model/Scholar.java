...
public class Scholar {
	...
	private Connection connect() {
		...
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/gadgetbadgetdb?useTimezone=true&serverTimezone=UTC",
					"root", "");
		} catch (Exception e) {
	...
	public String readresearchstatus() {
		...
		try {
			...
			//`refID``researchID``progress``comment``approval`
			while (rs.next()) {
	...
	public String updatesresearchstatus(String refID,String researchID,String progress,String comment,String approval) {
		String output = "";
		try {
			Connection con = connect();
			if (con == null) {
